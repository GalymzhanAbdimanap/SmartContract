...
contract MarketPlace{
   ...
   function saleOfGoodsBox(uint[] memory _amountOfgoods, uint[] memory _id_goods, string memory _timestamp)public {
       // Accept multi sale.
       _index = addIndex();
       
       for(uint i=0; i<_id_goods.length; i++){
          saleOfGoods(_id_goods[i], _amountOfgoods[i], _index, _timestamp);
           
       }
       setIndexOfChecks(msg.sender, _index);
   }
  
   function placeBidBox(uint[] memory _id_goods, uint[] memory _amounts, string memory _timestamp)public{
       // Accept multi purchase.
       _index = addIndex();
       
       for(uint i=0; i<_id_goods.length; i++){
           //a = Auction(address(_addresses[i]));
           placeBid(_id_goods[i], _amounts[i], msg.sender, _timestamp, _index);
   ...
   function finalizeBox(uint[] memory _id_goods, uint _index)public{
       // Confirm purchase.
        for(uint i=0; i<_id_goods.length; i++){
            //a = Auction(address(_addresses[i]));
            finalizeAuction(_id_goods[i], msg.sender, _index);
   }
   }
   
   //addresses = addresses auction
   function cancelConfirmBox(uint[] memory _id_goods, uint  _index)public{
       // Accept multi confirm.
       for(uint i=0; i<_id_goods.length; i++){
            cancelConfirm(_id_goods[i],msg.sender, _index);
       }
   }
   ...
   function balanceOf(address account)  external view returns(uint256){
    ...
    function createAuction(
        ...
        uint _commission) payable public{
            AllGoods[_index_goods].title = _title;
            AllGoods[_index_goods].startPrice = _startPrice;
            AllGoods[_index_goods].description = _description;
            AllGoods[_index_goods].count = _count;
            AllGoods[_index_goods].commission = _commission;
            aucStates[_index_goods] = State.Running;
            _index_goods+=1;
            
            
        }
    function returnContentsAuctions(uint _index_goods)public view returns(uint, string memory, uint, string memory, uint, uint){
        // Return content of good.
        return (_index_goods, AllGoods[_index_goods].title, AllGoods[_index_goods].startPrice, AllGoods[_index_goods].description, AllGoods[_index_goods].count, AllGoods[_index_goods].commission);
    }
        
    function returnTitleAuctions(uint _index_goods)public view returns(string memory){
        return AllGoods[_index_goods].title;
        
    }
    function returnStrtPrcAuctions(uint _index_goods)public view returns(uint){
        return AllGoods[_index_goods].startPrice;
        
    }
    function returnCountAuctions(uint _index_goods)public view returns(uint){
        return AllGoods[_index_goods].count;
        
    }
    function returnCommissionAuctions(uint _index_goods)public view returns(uint){
        return AllGoods[_index_goods].commission;  
    }
    ...
    function setCountAdd(uint _id_auc, uint _count) public {
        AllGoods[_id_auc].count = AllGoods[_id_auc].count.add(_count);   
    }
    
    function setCountSub(uint _id_auc, uint _count) public {
        AllGoods[_id_auc].count = AllGoods[_id_auc].count.sub(_count);   
    }

    // buy goods
    function placeBid(uint _id_goods, uint amt, address addr, string memory tmstp, uint id)  public notOwner returns(bool) {
        // Accept buy goods.
        string memory ttl = returnTitleAuctions(_id_goods);
        uint strtPrc = returnStrtPrcAuctions(_id_goods);
        uint cnt = returnCountAuctions(_id_goods);
        uint commission = returnCommissionAuctions(_id_goods);
        uint cnstPrc;
        cnstPrc = strtPrc;
        uint id_a = _id_goods;
     ...
     function finalizeAuction(uint _id_goods, address _address, uint _index) payable public{
        //confirm given good
        string memory ttl = returnTitleAuctions(_id_goods);
        uint strtPrc = returnStrtPrcAuctions(_id_goods);
        uint cnt = returnCountAuctions(_id_goods);
        uint cmsn = returnCommissionAuctions(_id_goods);
        uint cnstPrc;
        cnstPrc = strtPrc;
         
        //the owner and bidders can finalize the auction.
        require(_address == owner || bids[_id_goods][_address][_index] > 0);
        ...
        // Other bidders can get back the money 
            ...
            value =  bids[_id_goods][_address][_index];
        }
        // initialize the value
        bids[_id_goods][_address][_index] = 0;
        //recipiant.transfer(value);
        setNameOfGoodFromWallet(_address, _id_goods);
        
        settWalletOfGoods(_address, _id_goods, ttl, bids_goods[_id_goods][_address][_index], cnstPrc, cmsn);
        ...
        if(cnt<=0){
            aucStates[_id_goods] = State.Finalized;
        }
        
    }
    
    function cancelConfirm(uint _id_goods, address _address, uint _index) payable public{
        require(_address == owner || bids[_id_goods][_address][_index] > 0);
